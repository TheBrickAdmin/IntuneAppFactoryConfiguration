...
Try {
	...
	If ($deploymentType -ine 'Uninstall') {
		...
		$exeInstallParams = "/VERYSILENT /NORESTART"
		...
		## SAMPLE: PowerShell Script + parameter input
		#Execute-Process -Path "powershell.exe" -Parameters "-ExecutionPolicy Bypass -Command ""& '$($dirFiles)\ScriptName.ps1' -Parameter Input"""
		...
		#$msiInstall = "###SETUPFILENAME###"
		#$msiInstallParams = ""
		#Execute-MSI -Action "Install" -Path "$($dirFiles)\$($msiInstall)" -Parameters "/qn REBOOT=ReallySuppress" -AddParameter $msiInstallParams

		## SAMPLE: MSI + MST
		#$msiInstall = "###SETUPFILENAME###"
		#$mstInstall = "setup.mst"
		#Execute-MSI -Action "Install" -Path "$($dirFiles)\$($msiInstall)" -Transform "$($dirFiles)\$($mstInstall)" -Parameters "/qn REBOOT=ReallySuppress"

		## SAMPLE: MSI + MST + Params
		#$msiInstall = "###SETUPFILENAME###"
		#$mstInstall = "setup.mst"
		#$msiInstallParams = ""
		#Execute-MSI -Action "Install" -Path "$($dirFiles)\$($msiInstall)" -Transform "$($dirFiles)\$($mstInstall)" -Parameters "/qn REBOOT=ReallySuppress" -AddParameter $msiInstallParams	
	...
	ElseIf ($deploymentType -ieq 'Uninstall')
	{
		...
		# Execute-Process -Path "%ProgramFiles%\Greenshot\unins000.exe" -Parameters "/VERYSILENT /NORESTART"
